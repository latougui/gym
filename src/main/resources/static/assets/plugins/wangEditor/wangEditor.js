...
var config = {
    ...
    // 默认菜单配置
    menus: ['templates.admin.includes.head', 'bold', 'italic', 'underline', 'strikeThrough', 'foreColor', 'backColor', 'link', 'templates.list', 'justify', 'quote', 'emoticon', 'image', 'table', 'video', 'code', 'undo', 'redo'],
...
function DropList(menu, opt) {
    ...
    var list = opt.list || [];
    var type = opt.type || 'templates.list'; // 'list' 列表形式（如“标题”菜单） / 'inline-block' 块状形式（如“颜色”菜单）
    var onClick = opt.onClick || _emptyFn;

    // 加入 DOM 并绑定事件
    var $list = $('<ul class="' + (type === 'templates.list' ? 'w-e-list' : 'w-e-block') + '"></ul>');
...
function Head(editor) {
    ...
    this.droplist = new DropList(this, {
        width: 100,
        $title: $('<p>设置标题</p>'),
        type: 'templates.list', // droplist 以列表形式展示
